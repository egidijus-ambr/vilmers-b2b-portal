...
const OTTCURVEFULLINSIDEMANYPILLOWR = ({
  id,
  width,
  height,
  x,
  ...
  const interpolatePoints = (points, numberOfPillows, pillowSpacing) => {
    ...
    if (junctionIndex < interpolatedPoints.length && junctionIndex !== 0) {
      interpolatedPoints[junctionIndex].rotation = 45
  ...
  useEffect(() => {
    const points = [
      borderAdjustmentMarginWidth + backPillowSize / 2,
  ...
  return (
    <Group
      ...
      width={groupWidth}
      height={groupHeight}
      type="OTTCURVEFULLINSIDEMANYPILLOWR"
      x={xOffset}
      y={yOffset}
      originalWidth={width}
    ...
    >
      ...
      <Shape
        ...
        sceneFunc={(ctx, shape) => {
          ...
          ctx.rect(
            shapeWidth - shapeBackrestWidth - borderAdjustmentMarginWidth,
            borderAdjustmentMarginHeight,
            shapeBackrestWidth,
...
export default OTTCURVEFULLINSIDEMANYPILLOWR
